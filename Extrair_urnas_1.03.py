...
while True:
 ...
 try:
  if not(csv_city_name == False):
   ...
   csv_test.close()
   csv_city_encoding = input("Insira o encoding do arquivo, deixe em branco para o padrao(latin-1)")
   if csv_city_encoding == '':
    csv_city_encoding = 'latin-1'
  try:
   csv_city_file = open(csv_city_name, 'r', encoding=csv_city_encoding).readlines()
  except:
   print("Erro na leitura do arquivo, arqivo nao sera lido")
   csv_city_name = False
 except:
...
for zip_file in file_list:
 ...
 if py7zr.is_7zfile(zip_file):
  ...
  for line in range(48,55):
   if "Modelo de Urna" in file[line]:
